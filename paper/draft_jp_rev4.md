...
本シミュレーションで用いたポテンシャルパラメータ（基準値 $a=1.0, b=1.0$、性格特性による $a$ の $0.5 \sim 2.0$ への変動）およびノイズ強度（$\sigma=0.05 \sim 0.1$）の妥当性を確認するため、ウェアラブルセンサを用いたストレス計測公開データセット **WESAD (Schmidt et al. 2018)** [10] に基づく事後検証を行った。
