...
|:---:|:---:|:---:|:---|:---|
| 1 | 8.0 | 0.01 | $s = e + \dot{e}$（遅延あり） | 発散（Panic Spiral） |
| 2 | 0.5 | 0.3 | $s = e$（微分項なし） | 転落（Silent Submission） |
| 3 | 10.0 | 0.01 | $s = e$（微分項なし） | チャタリング |
| **4** | **5.0** | **0.3** | $s = e$（微分項なし） | **安定（99.9%）** |

### 5.2.1 モデルパラメータの妥当性検証 (Parameter Validation)
$a=2.0$はArnsten (2009) [7] の報告するPFCの回復時定数、$b=1.0$はScheffer et al. [2] の臨界減速現象に基づく。また、ノイズ強度（$\sigma=0.05 \sim 0.1$）およびポテンシャル形状については、ウェアラブルセンサを用いたストレス計測公開データセット **WESAD (Schmidt et al. 2018)** [9] による事後検証を行った。

WESADデータセットにおける心拍変動（HRV）の急激な低下および皮膚電気活動（EDA）のピーク到達時間（刺激開始から約2〜5分）を、状態変数 $x$ の「分水嶺（Tipping Point）を超えた転落プロセス」として解釈した結果、本モデルの時定数設定とオーダーレベルで一致することを確認した。具体的には、外乱（TSST：公衆心理ストレス試験）による生理指標の異常偏位と、本モデルにおいて外乱強度 $-2.5 \sim -4.0$ によって引き起こされるポテンシャル障壁の突破プロセスは、数理的に等価な動態を示す。
...
[4] M. Osman, "Controlling uncertainty: A review of human learning in complex dynamic environments," *Psychological Bulletin*, vol. 136, no. 1, pp. 65-86, 2010.
